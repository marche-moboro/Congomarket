...
'meuble': 'meubles', 'meubles': 'meubles'
  ...
  try {
    ...
    const [
      ...
      addVille(db.from(TABLES.SELLERS).select(SELLER_PUBLIC_COLS)
        .ilike('quartier', `%${q}%`)
        .eq('is_blocked', false)
        .eq('is_active',  true)),
...
async function openSellerProductsFromSearch(sellerId) {
  try {
    const { data: seller, error } = await db.from(TABLES.SELLERS)
     .select(SELLER_PUBLIC_COLS).eq('id', sellerId).maybeSingle();
